...
public class SortDao {
	...
	public ArrayList<Contact> sorting(int col, int uid) throws SQLException {
		ArrayList<Contact> alc = new ArrayList<>();

		if (col >= 1 && col < 5) {
			String query = "select * from contact_personel  where uid=? ORDER BY " + arr[col - 1];
			...
			String query = "select distinct(cid) from " + pea[col - 5] + " ORDER BY " + arr[col - 1];
			PreparedStatement stmt = con.prepareStatement(query);
			ResultSet rs = stmt.executeQuery();

			while (rs.next()) {
				if (displaypersonel(uid, rs.getInt("cid")) != null) {
